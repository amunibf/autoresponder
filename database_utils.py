...
def init_db():
    ...
    try:
        ...
        for i in range(1, 31):
            ...
            try:
                add_column_sql = f"ALTER TABLE subscribers ADD COLUMN {column_name} BOOLEAN DEFAULT FALSE;"
...
def confirm_subscriber(token):
    ...
    try:
        ...
        select_sql = """
        SELECT id, name, email, is_confirmed, token_expiry FROM subscribers
...
def get_all_confirmed_subscribers():
    ...
    try:
        ...
        select_sql = f"SELECT {', '.join(columns)} FROM subscribers WHERE is_confirmed = TRUE"
...
def get_subscriber_by_email(email):
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor() # get_db_connection sudah mengembalikan DictCursor secara default
        select_sql = "SELECT id, name, email, is_confirmed FROM subscribers WHERE email = %s"
...
def get_subscribers_count():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor() # Gunakan cursor default untuk operasi ini
        cursor.execute("SELECT COUNT(id) FROM subscribers WHERE is_confirmed = TRUE")
